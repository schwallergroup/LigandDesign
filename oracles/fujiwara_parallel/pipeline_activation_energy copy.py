    ...
    for variant in smiles_path.iterdir(): # Iterates through the variants 0-3
        if str(variant.stem) == '3':
            ...
            for file in files: # Iterates through each intermediate 0, 1, 2, 3 and ts
                ...
                if index != 'ts':
                    if index == '0': # For the initial catalyst, an unconstrained optimization is performed
                    ...
                    ts_constrained_opt_theory_level = Theory(
                        method = method,
                        solvent = solvent,
                        job_type = [OptimizationJob(constraints = {"atoms": fixed_atoms}), FrequencyJob(temperature = 373)]
                    )
                    ORCA_BLOCKS_constrained = {"geom": ["MaxIter 500"]}
...
def extract_energies(SMILES):
    ...
    with open (output_file, 'a') as f:
        ...
        compound_0_file = Path(SMILES_dir, '3/0_comput/catalyst.out')
...
def calculate_energy_single(smiles):
    ...
    return(extract_energies(arranged_smiles))

def calculate_energy(smiles: Union[str, List[str]]) -> Union[float, List[float]]:
    """
    If given one SMILES string → run single.
    If given a list of SMILES → run them in parallel (3 at a time).
    """
    if isinstance(smiles, str):
        # Single molecule case
        return calculate_energy_single(smiles)

    elif isinstance(smiles, list) or isinstance(smiles, tuple):
        results = [None] * len(smiles)

        # Run 3 at a time in parallel
        with ProcessPoolExecutor(max_workers=3) as executor:
            future_to_idx = {
                executor.submit(calculate_energy_single, s): i
            ...
            for future in as_completed(future_to_idx):
                ...
                try:
                    ...
                    results[idx] = 1000  # penalty for failed job
